...
namespace servo_sin_vel_demo {

struct CliOptions {
  apps_common::CommonOptions common;
  const char* eni_path = nullptr;
  std::int32_t amplitude_counts_per_sec = 100000;
...
void printUsage(const char* exe) {
  std::cerr
      ...
      << "\n"
      << "ENI-driven generic CiA402 velocity (CSV) sine demo. The ENI is validated against the\n"
      << "project-bundled ETG.2100 schema (embedded — no --xsd). Generate one from the CSV ESI\n"
      << "(STEPPERONLINE_A6_Servo_V0.04.csv.xml) with eni_gen before running. The EtherCAT\n"
      ...
      << "                   velocity 0x60FF and set modes-of-operation 0x6060 to CSV (9).\n"
      << "\n"
      << "Commands a sinusoidal CSV target velocity:\n"
      ...
      << "\n"
      << "Options:\n"
      << "  --amplitude N       Velocity amplitude in encoder counts/s (default: 100000).\n"
...
bool parseCli(int argc, char* argv[], CliOptions& opts) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (!std::strcmp(argv[i], "--auto-fault-recover")) opts.auto_fault_recover = true;
      ...
      opts.amplitude_counts_per_sec = static_cast<std::int32_t>(std::strtol(argv[++i], nullptr, 10));
...
int main(int argc, char* argv[]) {
  servo_sin_vel_demo::CliOptions opts;
  if (!servo_sin_vel_demo::parseCli(argc, argv, opts)) return 1;
  apps_common::installSignalHandler();

  if (opts.sync_trace_capacity > 0) {
    opts.sync_trace_window_ns =
        servo_sin_vel_demo::syncTraceWindowFromEnvIfUnset(opts.sync_trace_window_ns);
  }

  lxmstr::NetworkConfig cfg = servo_sin_vel_demo::make_network_config(opts);
  ...
  std::cout << "Operational. CSV velocity sine profile:\n"
            << "  amplitude (cnt/s) = " << opts.amplitude_counts_per_sec << "\n"
  ...
  /* Start at zero velocity (sin(0)=0) so the profile does not step away from rest at t=0. */
  ...
  while (net.isRunning() && !apps_common::interrupted()) {
    ...
    const double amp = static_cast<double>(opts.amplitude_counts_per_sec);
    ...
    drive->moveAtVelocity(static_cast<std::int32_t>(clamped));
  ...
  if (opts.sync_trace_capacity > 0) {
    ...
    if (opts.sync_trace_file && opts.sync_trace_file[0] != '\0') {
      ...
      if (!out) {
        ...
        servo_sin_vel_demo::printSyncTraceReport(rep, cfg.debug.sync_trace_window_ns, out);
        wrote_file = true;
    ...
    if (!wrote_file) {
      servo_sin_vel_demo::printSyncTraceReport(rep, cfg.debug.sync_trace_window_ns, std::cout);
    }
  }
